(* This is modification of HashArray.ML (Poly/ML) for LargeInt.int *)
(*
    Copyright (c) 2000
...
structure HashArrayLargeInt:>
sig
    type 'a hash
    
    val hash: int -> 'a hash
    val update: 'a hash * LargeInt.int * 'a -> unit
    val sub: 'a hash * LargeInt.int -> 'a option
    val delete: 'a hash * LargeInt.int -> unit
    
    val fold: (LargeInt.int * 'a * 'b -> 'b) -> 'b -> 'a hash -> 'b
end =
...
local
    ...
    datatype 'a namedOption = Empty | Deleted | Used of LargeInt.int * 'a
in

    fun hashValue vecLen i = Int.fromLarge(LargeInt.mod(i, (Int.toLarge vecLen)))
    ...
    fun op sub (Hash {entries = ref arr, ...}, name : LargeInt.int) : 'a option =
